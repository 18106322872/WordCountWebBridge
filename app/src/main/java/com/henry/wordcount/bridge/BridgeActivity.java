...
public class BridgeActivity extends Activity {
    ...
    static final String KEY_WORD_VERIFY = "word_verify";   // v1.0.55：Word 精确模式（默认开，与网页版"默认勾选"一致）
    ...
    private void doUpload(Uri uri, String name, String mime) {
        try {
            ...
            // v1.0.55：Word 精确模式偏好（默认开）。原生上传必须自己带 word_verify，
            //   否则网页里勾的框对桥接已上传的文件无效（此前手机永远走程序自有统计）
            SharedPreferences sp = getSharedPreferences(PREFS, MODE_PRIVATE);
            ...
            if (sizeBytes > CHUNK_THRESHOLD) {
                ...
                jobId = uploadChunked(base, name, data, wordVerify);
            } else {
    ...
    private static String uploadSingle(String urlStr, String fileName, String mime, byte[] data, boolean wordVerify) throws Exception {
        ...
        try (OutputStream os = conn.getOutputStream()) {
            ...
            // v1.0.55：Word 精确模式开关（默认开），服务端据此决定 .docx 是否走 Word COM
    ...
    private String uploadChunked(String base, String fileName, byte[] data, boolean wordVerify) throws Exception {
        ...
        // 全部到达 → commit 合并+启动统计
        setStatus("正在合并并启动统计…");
        return commitUpload(base, uid, fileName, totalChunks, wordVerify);
    }
    ...
    private String commitUpload(String base, String uid, String name, int total, boolean wordVerify) throws Exception {
